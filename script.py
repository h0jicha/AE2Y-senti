...
sys.path.append('./tweet')
from tweet import tweet_getter, preprocessor
...
if __name__ == '__main__':
    ...
    PATH_SENTIMENT_VALUES_MLASK = f'_data/values_mlask_{date}.txt'
    ...
    api = tweet_getter.do_auth()
    
    print('getting tweet ids...')
    for until in list_until:
        tweet_getter.get_tweet_ids(api, QUERY, PATH_IDS, count=100, until=until)
    print('getting tweet ids done')

    print('getting tweet texts...')
    tweet_getter.get_tweet_texts(api, PATH_IDS, PATH_TEXTS)
